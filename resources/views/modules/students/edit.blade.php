{{-- resources/views/modules/students/edit.blade.php --}}
...
<div class="mb-6">
    ...
    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Update information for {{ $student->full_name }}</p>
</div>

<!-- Student Form -->
<div class="bg-white dark:bg-gray-800 rounded-lg shadow">
    <form action="{{ route('students.update', $student) }}" method="POST" enctype="multipart/form-data" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6 flex items-center">
            <img src="{{ Storage::url($student->photo) }}" alt="{{ $student->full_name }}" class="w-20 h-20 rounded-full object-cover mr-4">
            <div>
                <p class="text-sm font-medium text-gray-900 dark:text-white">Current Photo</p>
                <p class="text-xs text-gray-500 dark:text-gray-400">Upload a new photo to replace this one</p>
            </div>
        </div>
        @endif
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Leave empty to keep current photo</p>
                    @error('photo')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Active Student</span>
                    </label>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Uncheck to mark student as inactive</p>
                </div>
            </div>
        </div>
