...
@section('page-title', 'Edit Stream')
@section('page-description', 'Update stream information')
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm">
    ...
    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100">Edit Stream: {{ $stream->name }}</h2>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Update the stream information</p>
            </div>
    ...
    <!-- Form -->
    <div class="p-6">
        <form data-spa-form method="POST" action="{{ route('streams.update', $stream) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" 
                           id="sort_order" 
                           name="sort_order" 
                           value="{{ old('sort_order', $stream->sort_order) }}"
                           min="0"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-maroon/50 focus:border-maroon bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                           placeholder="1"
                           required>
                ...
                <div>
                    ...
                    <select id="is_active" 
                            name="is_active"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-maroon/50 focus:border-maroon bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
                        <option value="1" {{ old('is_active', $stream->is_active) ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ !old('is_active', $stream->is_active) ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('is_active')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
